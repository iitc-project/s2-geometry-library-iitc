...
@Export public strictfp class S2LatLngRect implements Exportable, S2Region {
   ...
   */
  @NoExport public S2LatLngRect(final S2LatLng lo, final S2LatLng hi) {
  ...
  @NoExport public S2LatLngRect(R1Interval lat, S1Interval lng) {
    this.lat = lat;
    this.lng = lng;
    // assert (isValid());
  }

  public S2LatLngRect(Object lo, Object hi) {
    if(lo instanceof S2LatLng && hi instanceof S2LatLng) {
      lat = new R1Interval(((S2LatLng) lo).lat().radians(), ((S2LatLng) hi).lat().radians());
      lng = new S1Interval(((S2LatLng) lo).lng().radians(), ((S2LatLng) hi).lng().radians());
    } else if(lo instanceof R1Interval && hi instanceof S1Interval) {
      lat = (R1Interval) hi;
      lng = (S1Interval) lo;
    } else {
      throw new IllegalArgumentException("Unsupported constructor arguments.");
    }
  }
